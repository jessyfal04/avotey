	...
	$.ajax({
		...
		.done(function (e) {
			if (e.status == "success") {
				$("#txt-connexion-uuid").html(e.data.uuid);
				toggleConnected(true);
				setMessages("success", "Inscrit avec succès");
			} else {
				setMessages("danger", e.message);
			}
		})
	...
	$.ajax({
		...
		.done(function (e) {
			if (e.status == "success") {
				...
				setMessages("warning", "Scrutin créé avec succès, notifications en cours d'envoi");
				clearForms();
...
function publierResultats(scrutinId, choices, votes, systemeVote) {
	...
	if (privateKey == null) {
		setMessages("danger", "Clé privée introuvable, cloture annulée.");
		return;
	}
	...
	.done(function (e) {
		if (e.status == "success") {
			seeScrutins("consulterScrutins");
			setMessages("warning", "Scrutin cloturé & résultats publiés avec succès. Notifications en cours d'envoi.");
...
function voter() {
	...
	if (systemeVote == "uninominal") {
		...
		if (choice == "") {
			setMessages("danger", "Veuillez choisir un choix");
			return;
		}
	...
	$.ajax({
		...
		.done(function (e) {
			if (e.status == "success") {
				...
				seeScrutins("consulterScrutins");
				setMessages("success", "Vote enregistré avec succès");
				clearForms();
			} else {
				setMessages("danger", e.message);
			}
...
function sendNotification(typeNotif, scrutinId) {
	...
	.done(function (e) {
		if (e.status == "success") {
			seeScrutins("consulterScrutins");
			setMessages("success", `Notifications de ${typeNotif} envoyées avec succès`);
		} else {
			setMessages("danger", e.message);
		}
	})
